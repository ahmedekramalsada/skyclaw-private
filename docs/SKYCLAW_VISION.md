# SkyClaw — Vision & Architecture Spec
...
SkyClaw is a **cloud-native Rust AI agent runtime** that combines:
...
| Dimension | OpenClaw | ZeroClaw | SkyClaw |
|-----------|----------|----------|---------|
...
SkyClaw inherits ZeroClaw's trait-based design and adds cloud-native traits:

| # | Trait | Purpose | SkyClaw Addition |
|---|-------|---------|------------------|
...
This is SkyClaw's **primary differentiator**. The messaging app IS the control plane.

#### User Onboarding Flow (Zero SSH)
```
User sends message to SkyClaw bot on Telegram:
  "Hey, set up my agent"
...
User sends a .env file attachment → SkyClaw:
...
For files exceeding channel limits, SkyClaw generates **presigned URLs** to cloud object storage.
...
3. OAUTH LINK: SkyClaw sends OAuth URL → user clicks → callback → token stored
4. QR CODE:    SkyClaw sends QR image in chat → user scans → paired
5. MAGIC LINK: SkyClaw sends one-time link → user clicks → session established
```
...
SkyClaw takes a middle path between OpenClaw's open marketplace and ZeroClaw's compiled-in-only approach:

#### SkyHub (Skill Registry)
...
2. **Verified Skills**: From SkyHub with signatures + scanning (safe marketplace)
...
# api_key sourced from vault://skyclaw/anthropic/api_key

[memory]
backend = "postgres"
connection_string = "vault://skyclaw/db/connection_string"
...
# For local: key_file = "~/.skyclaw/vault.key"

[channel.telegram]
enabled = true
# token from vault://skyclaw/telegram/bot_token
allowlist = []
file_transfer = true
max_file_size = "50MB"
...
[channel.web]
enabled = true
cors_origins = ["https://app.skyclaw.io"]

[orchestrator]
backend = "kubernetes"
namespace = "skyclaw-agents"
auto_scale = true
min_replicas = 1
max_replicas = 10
